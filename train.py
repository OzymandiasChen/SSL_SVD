...
class Trainer():
	...
	def __init__(self, expName):
		...
		if(config.TRAIN_MODE in ['NaiveNN']):
			...
			# detach()
		elif(config.TRAIN_MODE in ['KD_SSL']):
	...
	def trainValidDataLoader(self):
		...
		# Unlabel Loading ... ...
		if(config.TRAIN_MODE in ['SSL', 'KD_SSL', 'DA_KD_SSL']):
	...
	def one_pass_train(self, train_loader, epochIndex, fo, unlabel_loader = None):
		...
		epoch_loss = 0.0
		epoch_acc = 0.0
		if config.TRAIN_MODE in ['KD_SSL', 'DA_KD_SSL', 'SSL']:
		...
		for step, (batch_x, batch_y) in enumerate(train_loader): # 只要迭代 就说明 train_loader里有东西
			...
			elif(config.TRAIN_MODE in ['KD_SSL']): # config.TRAIN_MODE in ['KD_SSL', 'DA_KD_SSL']
